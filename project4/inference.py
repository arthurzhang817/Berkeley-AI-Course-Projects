...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if (noisyDistance == None):
      ...
      for pos in self.legalPositions:
        if emissionModel[util.manhattanDistance(pos, pacmanPosition)] > 0:
          allPossible[pos] = self.beliefs[pos] * emissionModel[util.manhattanDistance(pos, pacmanPosition)]
  ...
  def elapseTime(self, gameState):
    ...
    for oldPos in self.legalPositions:
      for newPos, prob in self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)).items():
...
class ParticleFilter(InferenceModule):
  ...
  def elapseTime(self, gameState):
    ...
    for i in range(0, len(particles)):
      newParticles.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, particles[i]))))
